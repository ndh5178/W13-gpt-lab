...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        if causal_mask:
            mask = torch.triu(torch.ones(seq_len, seq_len, device=x.device, dtype=torch.bool), diaonal=1)
        ...
        context = context.transpose(1, 2).contiquos().view(batch_size, seq_len, d_model)
